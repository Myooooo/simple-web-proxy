...
while True:
  ...
  # Error handling for file not found in cache
    ...
    if fileExists:
      ...
      clientResponse = version + ' 500 Internal Server Error'
      # ~~~~ END CODE INSERT ~~~~
      ...
      # ~~~~ INSERT CODE ~~~~
      # 5 seconds timeout to prevent blocking
      ...
      try:
        ...
        while True:
          tmp = originServerSocket.recv(1024)
          if not tmp:
            break
          else:
            originServerResponse.append(tmp)
        # ~~~~ END CODE INSERT ~~~~

        # Send the response to the client
        # ~~~~ INSERT CODE ~~~~
        for buf in originServerResponse:
          clientSocket.send(buf)
        # ~~~~ END CODE INSERT ~~~~

        # finished sending to origin server - shutdown socket writes
        originServerSocket.shutdown(SHUT_WR)
        ...
        print 'origin server request failed. '
        print message
  try:
    clientSocket.close()
  except:
